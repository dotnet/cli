...
namespace Microsoft.DotNet.ProjectModel
{
    public class ProjectReader
    {
        ...
        public Project ReadProject(Stream stream, string projectName, string projectPath, ICollection<DiagnosticMessage> diagnostics, ProjectReaderSettings settings = null)
        {
            ...
            project.Authors = rawProject.Value<JToken>("authors")?.ToObject<string[]>() ?? EmptyArray<string>.Value;
            project.Owners = rawProject.Value<JToken>("owners")?.ToObject<string[]>() ?? EmptyArray<string>.Value;
            project.Tags = rawProject.Value<JToken>("tags")?.ToObject<string[]>() ?? EmptyArray<string>.Value;
            ...
            if (scripts != null)
            {
                foreach (var script in scripts)
                {
                    ...
                    var arrayValue = script.Value.Type == JTokenType.Array ? script.Value.ToObject<string[]>() : null;
                    if (arrayValue != null)
                    {
            ...
            bool isGacOrFrameworkReference)
        {
            ...
            if (dependencies != null)
            {
                foreach (var dependency in dependencies)
                {
                    ...
                    {
                        throw FileFormatException.Create(
                            string.Format("Invalid dependency version: {0}. The format is not recognizable.", dependency),
                            dependencyValue,
                            projectPath);
                    }
        ...
        private static CommonCompilerOptions GetCompilationOptions(JObject rawObject, Project project)
        {
            ...
            {
                Defines = rawOptions.Value<JToken>("define")?.ToObject<string[]>(),
                SuppressWarnings = rawOptions.Value<JToken>("nowarn")?.ToObject<string[]>(),
                AdditionalArguments = rawOptions.Value<JToken>("additionalArguments")?.ToObject<string[]>(),
