...
bool pal::get_default_breadcrumb_store(string_t* recv)
{
    ...
    {
        trace::info(_X("Breadcrumb store not obtained because dotnet extensions could not be obtained"));
        return false;
    }
...
bool pal::get_default_extensions_directory(string_t* recv)
{
    ...
    if (!pal::directory_exists(ext))
    {
        trace::info(_X("Directory dotnet extensions at [%s] was not found"), ext.c_str());
        ext.clear();
        append_path(&ext, _X("opt"));
        append_path(&ext, _X("dotnetextensions"));
        if (!pal::directory_exists(ext))
        {
            trace::info(_X("Fallback directory dotnet extensions at [%s] was not found"), ext.c_str());
            return false;
        }
    }
    ...
    {
        trace::info(_X("Directory dotnet extensions at [%s] was not ACL-ed properly"), ext.c_str());
        return false;
    }
