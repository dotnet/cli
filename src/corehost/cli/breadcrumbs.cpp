...
bool breadcrumb_writer_t::write_worker_callback(breadcrumb_writer_t* p_this)
{
    bool retval = false;
    try
    {
...
bool breadcrumb_writer_t::end_write()
{
    if (m_thread.joinable())
    {
        ...
        {
            m_status = future.get();
            trace::verbose(_X("The breadcrumb worker thread has exited, and shared state is %d."), m_status);
        }
        else
        {
            m_status = false;
            trace::verbose(_X("The breadcrumb worker thread has exited, but shared state is not signaled."));
        }
    }
