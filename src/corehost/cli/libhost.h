...
{
    int32_t version_lo;                // Just assign sizeof() to this field.
    int32_t version_hi;                // Breaking changes to the layout -- increment HOST_INTERFACE_LAYOUT_VERSION
    ...
    // !! 4. Must be POD types; use non-const sized-integral and pointer types; no access modifiers.
...
#define HOST_INTERFACE_LAYOUT_VERSION_HI 0x8001
...
struct hostpolicy_init_t
{
    ...
    static bool init(host_interface_t* input, hostpolicy_init_t* init)
    {
        ...
        trace::verbose(_X("Reading from host interface version: [%d] to initialize policy version: [%d]"), input->version_lo, HOST_INTERFACE_LAYOUT_VERSION_LO);
