 ...
 * relative hostpolicy.dll exists
 */
...
/**
 * Given own location, FX location, app dll and specified --depsfile return
 *     location that is expected to contain hostpolicy
 */
bool get_expected_hostpolicy_dir(
    host_mode_t mode,
    ...
    pal::string_t* expected)
{
    ...
    {
        // Portable apps resolve hostpolicy from FX location
...
{
    // Try to obtain the expected hostpolicy location for the given configuration.
    pal::string_t expected;
    if (!get_expected_hostpolicy_dir(mode, own_dir, fx_dir, app_candidate, 
            deps_file, config, &expected))
    {
        return false;
    }
    ...
    if (!probe_realpaths.empty() && resolve_hostpolicy_dir_from_probe_paths(version, probe_realpaths, &candidate))
    {
