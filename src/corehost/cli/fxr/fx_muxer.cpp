...
pal::string_t fx_muxer_t::resolve_fx_dir(const pal::string_t& muxer_dir, runtime_config_t* runtime)
{
    ...
    const auto roll_fwd = runtime->get_patch_roll_fwd();
    ...
    if ((!specified.is_prerelease() && !roll_fwd))
    {
        trace::verbose(_X("Did not roll forward because rollfwd=%d"), roll_fwd, fx_ver.c_str());
    ...
    else
    {
        ...
        for (const auto& version : list)
        {
            ...
            fx_ver_t ver(-1, -1, -1);
            if (!specified.is_prerelease() && fx_ver_t::parse(version, &ver, true) &&
                ver.get_major() == max_specified.get_major() &&
                ver.get_minor() == max_specified.get_minor())
            {
                max_specified = std::max(ver, max_specified);
            }
            if (specified.is_prerelease() && fx_ver_t::parse(version, &ver, false) &&
