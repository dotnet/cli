...
__PACKAGES_DIR=$__scriptpath/.nuget/packages
__TOOLRUNTIME_DIR=$__scriptpath/build_tools
__DOTNET_PATH=$__scriptpath/.dotnet_stage0
...
__CHANNEL=$(awk -F "=" '/CHANNEL/ {print $2}' "$__scriptpath/branchinfo.txt")
...
if [ ! -e "$__PROJECT_JSON_FILE" ]; then
    "$__scriptpath/scripts/obtain/dotnet-install.sh" --install-dir "$__DOTNET_PATH" --channel $__CHANNEL

    if [ ! -d "$__PROJECT_JSON_PATH" ]; then mkdir -p "$__PROJECT_JSON_PATH"; fi
