...
"@
    ...
    if($Offline){
        ...
        if (!(Test-Path "$DnxDir\__version_$DnxVersion")){
            ...
            "" > "$DnxDir\__version_$DnxVersion"
        }
        else{
            header "DNX $DnxVersion already installed"
        }
    ...
    cp "$PSScriptRoot\dotnet-dnx.cmd" "$Stage2Dir\bin\dotnet-dnx.cmd"
    ...
    # Package Command Test of stage2
    & "$PSScriptRoot\test\package-command-test.ps1"
    if (!$?) {
        Write-Host "Command failed: $PSScriptRoot\test\package-command-test.ps1"
        Exit 1
    }
