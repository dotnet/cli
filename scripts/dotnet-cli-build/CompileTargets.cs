...
/*
            if (PlatformServices.Default.Runtime.OperatingSystemPlatform == Platform.Windows)
            {
                foreach (var pdbReader in Directory.GetFiles(Path.Combine(outputDir, "runtimes", "win", "native")))
                {
                    if (pdbReader.Contains("Microsoft.DiaSymReader.Native"))
                    {
                        c.Warn("Copying Microsoft.DiaSymReader.Native dll to app base: {pdbReader}");
                        File.Copy(pdbReader, Path.Combine(outputDir, Path.GetFileName(pdbReader)));
                    }
                }
            }
*/
