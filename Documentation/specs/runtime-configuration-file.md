...
### `version` Property (`.deps.json`)

This is a simple integer version indicating the version of the deps file format in use. It begins at `2` because the deps file format closely matches the format for `project.lock.json` which is at version `2`. In general, hosts should fail to load applications that use deps files with "unsupported" versions. It's up to the host to determine it's support policy. There is no guarantee of compatibility between versions of the deps file, each version could be completely different and require quirking to load properly, so no assumption should be made about the version field. The goal of this field is to allow us to make major changes without causing undefined behavior in existing hosts (instead providing a clearer "This application is not compatible with this host" error).

In the current version `corehost` should read this property and if it is not exactly `2`, it should produce an error indicating that the application targets a later version of the host. When/if a version `3` is introduced, a newer version of `corehost` will be able to understand this new format as well as the old format and switch it's behavior based on this version number.
