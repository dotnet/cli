...
The .NET Core Tools telemetry feature is *NOT* enabled by default. You can opt-in of the telemetry feature by setting an environment variable DOTNET_CLI_TELEMETRY_OPTIN (for example, 'export' on macOS/Linux, 'set' on Windows) to true (for example, 'true', 1). You can read more about .NET Core tools telemetry at https://aka.ms/dotnet-cli-telemetry.
