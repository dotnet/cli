...
namespace Microsoft.Dotnet.Tools.Test.Tests
{
    public class GivenThatWeWantToUseDotnetTestE2EInDesignTimeForMultipleTFms : TestBase
    {
        ...
        private string Net45OutputPath => Directory.EnumerateDirectories(_net451OutputPath).First();
        ...
        public GivenThatWeWantToUseDotnetTestE2EInDesignTimeForMultipleTFms()
        {
            ...
            var testInstance = testAssetManager.CreateTestInstance(Path.Combine("ProjectsWithTests", "MultipleFrameworkProject"));
            ...
            _net451OutputPath = Path.Combine(testInstance.TestRoot, "bin", "Debug", "net451");
        ...
        [Fact]
        public void It_discovers_tests_for_the_ProjectWithTestsWithNet45()
        {
            var outputPath = Directory.EnumerateDirectories(_net451OutputPath).First();
            using (var adapter = new Adapter("TestDiscovery.Start"))
            {
                adapter.Listen();

                var testCommand = new DotnetTestCommand();
                var result = testCommand.Execute($"{_projectFilePath} -f net451 -o {Net45OutputPath} --port {adapter.Port} --no-build");
                result.Should().Pass();
        ...
        [Fact]
        public void It_runs_tests_for_net45()
        {
            using (var adapter = new Adapter("TestExecution.GetTestRunnerProcessStartInfo"))
            {
                adapter.Listen();
