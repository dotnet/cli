...
namespace Microsoft.Dotnet.Tools.Test.Tests
{
    public class GivenThatWeWantToRunTestsForMultipleTFMsInTheConsole : TestBase
    {
        ...
        public GivenThatWeWantToRunTestsForMultipleTFMsInTheConsole()
        {
            ...
            var testInstance =
                testAssetManager.CreateTestInstance(Path.Combine("ProjectsWithTests", "MultipleFrameworkProject"), identifier: "ConsoleTests");
            ...
            _defaultNet451OutputPath = Path.Combine(testInstance.TestRoot, "bin", "Debug", "net451");
        }

        private string Net45OutputPath => Directory.EnumerateDirectories(_defaultNet451OutputPath).First();
        ...
        public void It_skips_build_when_the_no_build_flag_is_passed_for_net451()
        {
            ...
            var testCommand = new DotnetTestCommand();
            result = testCommand.Execute($"{_projectFilePath} -f net451 -o {Net45OutputPath} --no-build");
            result.Should().Pass();
        }
        ...
        public void It_prints_error_when_no_framework_matched()
        {
            ...
            result.Should().Fail();
            result.StdErr.Should().Contain($"Project does not support framework");
        }
