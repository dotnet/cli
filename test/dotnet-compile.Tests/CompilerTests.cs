...
namespace Microsoft.DotNet.Tools.Compiler.Tests
{
    public class CompilerTests : TestBase
    {
        ...
        public void XmlDocumentationFileIsGenerated()
        {
            ...
            var buildCommand = new BuildCommand(testProject, output: outputDir, framework: DefaultLibraryFramework, runtime: "win7-x64");
        ...
        public void SatelliteAssemblyIsGeneratedByDotnetBuild()
        {
            ...
            var outputDir = Path.Combine(testLibDir.Path, "bin");
            var testProject = GetProjectPath(testLibDir);
            var buildCmd = new BuildCommand(testProject, output: outputDir, framework: DefaultFramework, runtime: "win7-x64");
        ...
        public void LibraryWithAnalyzer()
        {
            ...
            var outputDir = Path.Combine(testLibDir.Path, "bin");
            var testProject = GetProjectPath(testLibDir);
            var buildCmd = new BuildCommand(testProject, output: outputDir, framework: DefaultLibraryFramework, runtime: "win7-x64");
            var result = buildCmd.ExecuteWithCapturedOutput();
            result.Should().Pass();
